  ...
  for (var i=0; i<obj.entry.length; i++) {
    ...
    if (e.video == true) {
      ...
      if (e.image)
        ...
        img_attr = 'src="icons/Movies-icon.png"';
    }
    else {
      link_attr = String.format('href="list.html?p={0}" onclick="onPageClick.call(this);" title="{1}"', e.link, e.title);
      if (e.image)
        img_attr = String.format('src="{0}" onerror=\'this.onerror=null; this.src="icons/playlist-icon.png"\'', e.image);
      else
        img_attr = 'src="icons/playlist-icon.png"';
    }
...
function result_play(obj) {
  var text = '';
  text += String.format('<h2>{0} <a href="{0}" target="_blank">{1}</a></h2>', getLangLog('playing'), obj.video);
  return text;
}
